...
# FUNCTIONS
def set_svo_pvars(player: Player):
    svo_values = C.svo_values
    question_to_pay = randint(1, 6)
    value = getattr(player, f'q{question_to_pay}')
    player.participant.vars['svo_temp_question_selected'] = dict(
        num=question_to_pay,
        ego_payoff=svo_values[f'C1{question_to_pay}'][value],
        alter_payoff=svo_values[f'C2{question_to_pay}'][value]
    )
...
class ResultWaitPage(WaitPage):
    ...
    def after_all_players_arrive(group: Group):
        for p in group.get_players():
            o = p.get_others_in_group()[0]
            p.participant.svopayoff = (svo_values['C11'][p.q1 - 1] +
                    svo_values['C12'][p.q2 - 1] +
                    svo_values['C13'][p.q3 - 1] +
                    svo_values['C14'][p.q4 - 1] +
                    svo_values['C15'][p.q5 - 1] +
                    svo_values['C16'][p.q6 - 1]) / 100

            o.participant.svopayoff = 6 - (svo_values['C11'][p.q1 - 1] +
                    svo_values['C12'][p.q2 - 1] +
                    svo_values['C13'][p.q3 - 1] +
                    svo_values['C14'][p.q4 - 1] +
                    svo_values['C15'][p.q5 - 1] +
...
page_sequence = [StartWaitPage, SVO, ResultWaitPage, Results]
